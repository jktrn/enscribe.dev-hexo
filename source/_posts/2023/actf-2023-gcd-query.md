...
{% info %}
**Notice**: I'm in the process of moving this blog to NextJS! For a better reading experience, I suggest you check out the [**updated version of this website**](https://test-blog-jktrn.vercel.app/blog/actf-2023-gcd-query), which is temporarily deployed on Vercel.
{% endinfo %}
...
![2048-bit Visual](/static/actf-2023/2048.svg)
...
![Source Code Graphical Visualization](/static/actf-2023/visualization.svg)
...
{% theorem %}
**Chinese Remainder Theorem**: Given pairwise coprime integers $n\_1, n\_2, ..., n\_k$ and arbitrary integers $a\_1, a\_2, ..., a\_k$, the system of simultaneous congruences

$$
\begin{cases}
    x \equiv a\_1 \pmod{n\_1} \\\\
    x \equiv a\_2 \pmod{n\_2} \\\\
    \vdots \\\\
    x \equiv a\_k \pmod{n\_k}
\end{cases}
$$

has a solution, and the solution is unique modulo $N = n\_1 n\_2 \cdots n\_k$.
{% endtheorem %}

{% info %}
Although the Chinese Remainder Theorem is often stated with pairwise coprime moduli (meaning that for a set of moduli $M = \\{n\_1, n\_2, \ldots, n\_k\\}$, $\gcd(n\_i, n\_j) = 1$ for all $i \neq j$), it can be extended to non-coprime moduli. However, doing so does not guarantee a solution — this will become increasingly relevant as we get towards our implementation process.
{% endinfo %}
...
![Carnival Game](/static/actf-2023/carnival-visual.svg)
...
Bob goes back to his table. He's flabbergasted. How in the world is he going to guess that number with only three pieces of information? He rummages around his little noggin and recollects himself. Let's see what he's thinking:

![Bob Thinking](/static/actf-2023/bob-thinking.svg)
...
{% info %}
Note: Bob's chosen negative values for $n\_2$ and $n\_3$ because of the earlier relation established, $x \equiv -n \pmod{\gcd(x + n, m)}$. Making $$n$$ negative creates positive remainders.
{% endinfo %}

For $m$, Bob chooses a very large **primorial**:

{% definition %}
**Primorial**: For the $n$th prime number $p\_n$, the primorial $p\_n\\#$ is defined as the product of the first $n$ primes:
    
$$
p\_n\\# = \prod_{k=1}^n p\_k
$$

where $p\_k$ is the prime number.
{% enddefinition %}
...
Bob's ended up deciding on $m = p\_{11}\\#$. He pulls out his laptop and calculates it with Python:

```py Calculating Primorials
...
![Bob Handing Papers](/static/actf-2023/handing-papers.svg)
...
```py Solving the Carnival Puzzle
...
![Congrats, Bob!](/static/actf-2023/congrats-bob.svg)
...
{% ccb terminal:true html:true scrollable:true wrapped:true %}
...
{% ccb lang:python gutter1:1-10,+,11-16,+,17-30 gutter2:1-9,⁠—,10-11,⁠—,12-14,⁠—,15-29 caption:'gcd-query-v2 Solve Attempt' diff_add:11,18 diff_del:10,13,17 %}
from sympy.ntheory.modular import crt
from sympy import primorial
from tqdm import tqdm
from pwn import *
...
{% ccb lang:python gutter1:⁠6,7,⁠+x2,8 gutter2:⁠—x2,6-8 caption:'Changing Primorial' diff_add:3,4 diff_del:1,2 %}
# Generate the 256th primorial
N = 2**8
...
Thanks to everyone from [les amateurs](https://ctf.amateurs.team/) for hosting this CTF! I had a lot of fun solving these challenges and I hope to see more from you guys in the future. I'd also like to credit Quasar, [SuperBeetleGamer](https://www.cryptohack.org/user/SuperBeetleGamer/), and [flocto](https://ctftime.org/user/121085) for helping me wrap my head around CRT in general throughout the process of writing this (because I almost always learn along the way). I hope you learned something like I did!
